<?php
	...
	class FundModel extends AdminBaseModel
	{
		//得到用户数据
		...
		public function add_log  ($array) {
			$add['users_id'] = $array['users_id'];
			$add['money'] = $array['money'];
			$add['adormed'] = $array['adormed'];
			$add['shop_number'] = $array['shop_number'].time();
			$add['type'] = $array['type'];
			$add['member_info'] = $array['member_info'];
			$add['admin_info']  = $array['admin_info'];
			$add['time'] = time();
			$add['status'] = 1;
			return $this->add($add);
		}
