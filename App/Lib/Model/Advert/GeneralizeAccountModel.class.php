	...
	class GeneralizeAccountModel extends AdvertBaseModel
	{
			...
			if($zhifu_id!='')
			{
				...
				if($account_money['money'] < $price)
				{
					...
					if($bool)
					{
						...
						D('Fund')->djFund($account_id,$price,$zhifu_id,3);
		...
		public function getUserPr($small_order_id,$users_id)
		{
			if($small_order_id!='' && $users_id!='')
			{
				...
				$UserMedia = D('UserMedia');
				$UserMedia->insertPirce($all_array['users_id'],$all_array['price'],3,$all_array['generalize_id']);
				//计算折扣*原价从广告主冻结资金里面扣除
				$sum_price = $all_array['price'] + ($all_array['price'] * $all_array['rebate']);
				$bool = D('UserAdvertisement')->updateFreeze($users_id,$sum_price,3,$all_array['generalize_id']);
				if($bool)
				{
