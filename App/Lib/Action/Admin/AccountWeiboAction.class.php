...
class AccountWeiboAction extends AdminBaseAction {
	...
	protected  $db = array(
		...
		'GrassrootsWeibo'=>'GrassrootsWeibo',	//微博草根索引表
		'CeleprityindexWeibo'=>'CeleprityindexWeibo',//微博微博名人索引表
	);
		...
		if($act == 'recommended') {
			...
			if ($this->isPost()) {
				...
				if ($this->_post('is_celebrity') == 0) {
					$this->db['GrassrootsWeibo']->create();
					$this->db['GrassrootsWeibo']->where(array('weibo_id'=>$id))->save();
