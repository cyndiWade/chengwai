 ...
 * 站内短信控制器
 */
class MessageAction extends AdvertBaseAction  {
	...
	*/
	public function index()
	{
        ...
        parent::data_to_view(array(
				//二级导航属性
			'sidebar_order'=>array($type=>'select'),//第一个加依次类推
		));
	...
	*/
	function show()
	{
		...
		if($id)
		{
			$info = D('Media/Message')->field('`subject`, `content`')->where(array("messageid"=>$id))->find();
			if($info)
			{
				$where['messageid']	= $id;
				$data['readed']		= 1;
