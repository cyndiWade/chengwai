define('weiboyi', function (require) {
	...
	(function ($, W, document, doT, undefined) {
		...
		var opts = {
			...
			selectedRow : [],
			autoLoad : true,
			cmpCls : "weiboyiGrid tab01-account-admin l",
			messages : {
		...
		var methods = {
			initComponent : function () {
				...
				this.tableDom.className = "tab01-account-admin l";
				this.buildHead();
				...
				var self = this,
				start = this.pageInfo.start || 0,
				limit = this.pageInfo.limit || 20,
				total = 0;
				if (!this.pagebar) {
			...
			doEditable : function () {
				...
				self.el.on("click", "tr .weiboyiGrid_editableCell", function (e) {
					...
					if (row) {
						...
						editor.val(row.cells[el.attr("data-col")]).css({
				...
				$.each(self.columns || [], function (i, col) {
					...
					if (col.opts && col.opts instanceof Array) {
						...
						if (editable) {
							...
							var cell = $("<span class='weiboyiGrid_cellContent'></span>");
							cell.appendTo(td);
