define(function(require) {
    ...
    ko.extenders.area = function(target, cfg) {
        ...
        target.country_option = ko.observableArray([{id:1, name: '中国'}]);
        ...
        target.country.subscribe(function(newCountry){
            if(target.district() && newCountry == parseInt(target.district().toString().substr(0,(''+newCountry).toString().length))){
                return;
            }
            if(newCountry){
        ...
        target.province.subscribe(function(newProvince){
            if(target.district() && newProvince == parseInt(target.district().toString().substr(0,(''+newProvince).toString().length))){
                return;
            }
        ...
        target.city.subscribe(function(newCity){
            if(target.district() && newCity == parseInt(target.district().toString().substr(0,(''+newCity).toString().length))){
                return;
            }
            if(newCity){
